...
const LevelManager = {
  ...
  // Дані для екрана "усі рівні й нагороди": масив {level, reward, rankStart}
  // для 1..MAX_REWARD_LEVEL, rankStart містить об'єкт звання, якщо на цьому
  // рівні воно змінюється.
  getRewardTable() {
    const rows = [];
    for (let lvl = 1; lvl <= MAX_REWARD_LEVEL; lvl++) {
      const rankStart = RANKS.find(r => r.min === lvl) || null;
      rows.push({ level: lvl, reward: lvl === 1 ? 0 : this.rewardForLevel(lvl), rankStart });
    }
    return rows;
  },  // Додає XP і, якщо перетнули один чи кілька рівнів (<=100), нараховує
  // монетну нагороду за кожен пройдений рівень.
  addXp(amount) {
    const inc = Math.max(0, Math.floor(amount));
    ...
    if (afterLevel > beforeLevel) {
      for (let lvl = beforeLevel + 1; lvl <= Math.min(afterLevel, MAX_REWARD_LEVEL); lvl++) {
        rewardCoins += this.rewardForLevel(lvl);
      }
      if (rewardCoins > 0) CoinBank.addSilent(rewardCoins);
    }
    return { beforeLevel, afterLevel, leveledUp: afterLevel > beforeLevel, rewardCoins };
  }
};
...
// Спливаюче сповіщення про отриманий за досягнення XP.
function showXpToast(items) {
  ...
  items.forEach((item, i) => {
    setTimeout(() => {
      ...
      card.textContent = `${item.icon || '🏆'} ${item.name} · +${item.xp} XP`;
...
(function grantAchievementXpOnLoad() {
  ...
  if (newly.length) showXpToast(newly);
})();
...
window.Engine = { SoundFX, Particles, ScreenShake, Loop, lerp, clamp, aabb, mountMuteButton, mountFullscreenButton, mountCoinBadge, CoinBank, THEMES, ThemeManager, DailyBonus, Achievements, LevelManager };
