...
// Кнопка "на весь екран" — розгортає весь документ через Fullscreen API.
function mountFullscreenButton(container) {
  ...
  const paint = () => { btn.textContent = isFs() ? '⤦' : '⛶'; btn.title = isFs() ? 'Вийти з повного екрана' : 'На весь екран'; };
  paint();
...
window.Engine = { SoundFX, Particles, ScreenShake, Loop, lerp, clamp, aabb, mountMuteButton, mountFullscreenButton, mountCoinBadge, CoinBank, THEMES, ThemeManager, DailyBonus, Achievements };
