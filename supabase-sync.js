...
(function () {
  ...
  const KNOWN_KEYS = [
    'koinzal_coins', 'koinzal_total_earned', 'koinzal_theme',
    'koinzal_owned_themes', 'koinzal_daily_date', 'koinzal_daily_streak'
  ];
  ...
  function applyRemote(obj) {
    ...
    Object.keys(obj).forEach(k => {
      ...
      const isNumeric = k.endsWith('_best') || k === 'koinzal_coins' ||
        k === 'koinzal_total_earned' || k === 'koinzal_daily_streak';

      if (isNumeric) {
        const rv = Number(remote) || 0, lv = Number(local) || 0;
        try { localStorage.setItem(k, String(Math.max(rv, lv))); } catch (e) {}
      } else if (k === 'koinzal_owned_themes') {
        try {
          const remoteList = JSON.parse(remote || '["default"]');
          const localList = JSON.parse(local || '["default"]');
